...
class App extends React.Component {
    ...
    subscribe = async () => {
        ...
        let rec =
            {
                notifactionEmail: mail,
                anyTag: this.state.isAllTags,
                anyCountries: this.state.isAllCountries,
                minPrice: this.state.value[0],
                msxPrice: this.state.value[1],
                tags: this.state.selectedTags.map(e => ({'name': e.title})),
                countries: this.state.selectedCountries.map(e => ({'name': e.title})),
                minDuration: this.state.duration[0],
                maxDuration: this.state.duration[1]
            }

		//
		let startDate =  this.state.selectionRange.startDate.toISOString().substr(0, 10)
		let endDate = this.state.selectionRange.startDate.toISOString().substr(0,10)
