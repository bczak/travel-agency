...
class App extends React.Component {
	...
	search = async (param) => {
		...
		let res = await API.getTrips(this.state.selectedTags.map((e) => e.title), this.state.isAllTags,
			this.state.selectedCountries.map((e) => e.title), this.state.isAllCountries)
		let cards = []
		...
		return (
			<div id="App">
						...
						<div className={'date'}>
							...
							<div className="params">
								...
								<Switch
									...
									onChange={value => {
										this.setState({isAllCountries: !this.state.isAllCountries})
									}}
				...
				<Paper className={'result'} variant={'outlined'} elevation={2}>
					...
					<Paper className={'res'} elevation={3} variant={'outlined'}>{this.state.cards.length} results</Paper>
